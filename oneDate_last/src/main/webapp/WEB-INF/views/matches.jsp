	...
	<div data-enhance="false" class="container">
		...
		<div class="messaging">
			<div class="inbox_msg">
				<div class="mobile">
					<nav class="Mnavbar">
					<span onclick="location.href = 'toMain?userId=${sender}'" ><i class="fa fa-3x fa-random"></i></span>
					<span  style="color: #e33988"><i class="fa fa-3x fa-comments"></i></span>
					<span></span>
					</nav>
				</div>
										...
										<p>
											...
											<c:if
												test="${last_messagge_list.get(item.id).getUser_receive() eq sender }">
													<i style="float: right; color: red;" class="fa fa-paper-plane"></i>
											</c:if>
										</p>
					...
					<div class="tinder--cards">
						<c:forEach var="item" items="${candidate_list}">
							<div id="${item.idx}" class="tinder--card">
								...
								<i class="fa fa-map-marker">  ${age_Location_map.get(item.id).getDistance()}km</i>
								<p>${item.details}</p>
